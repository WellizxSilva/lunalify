...
local function _dispatch(title, message, options)
    ...
    return
        core.toast(
            title,
            message,
            options.app_id or settings.app_id,
            icon,
            hero,
            options.sound or "Default",
            options.silent or false,
            options.duration,
            options.scenario,
            options.actions or {},
            options.inputs or {},
...
function lunalify.kit.register(app_id, display_name, icon_path, daemon_log_path)
    ...
    local ok, err_msg, err_code = core.register(app_id, display_name, final_icon_path, final_daemon_log_path)
...
--- @param mode string|boolean The duration mode (see lunalify.CONSTANTS.DURATION for available options)
...
function API:update(value, status, display_value, title)
    ...
    return core.update_toast(
        self.data.opts.app_id or settings.app_id,
        self.data.opts.tag or self._generated_id,
        p.title or "",
        p.value or "0",
        p.displayValue or "",
        p.status or ""
...
--- @param callback function? Function that will be called before each iteration to check if the loop should stop. (should return `false` to stop)
...
function lunalify.kit.setup(config)
    ...
    local ok, err = lunalify.kit.register(settings.app_id, settings.app_name, settings.icon_path,
        log.daemon_log_path)
    if not ok then
        return false, err
    end
