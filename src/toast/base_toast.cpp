...
namespace Lunalify {
    Lunalify::Errors::Code MapHResultToLunalifyError(winrt::hresult hr) {
        ...
        switch (code) {
            ...
            case 0x80042001:
            case 0x80042008:
            case 0x803E0102:// Query Error (It's not a parsing error, but we treat it as a generic XML error)
            ...
            // Specific notification errors (WinRT)
            case 0x80070490: // Element not found (AppId not registered)
                return Lunalify::Errors::Code::WINRT_REGISTRATION_MISSING;

            case 0x803E0111: // Notifications are disabled by group policy
                return Lunalify::Errors::Code::WINRT_DISABLED_BY_POLICY;

            case 0x803E010B: // Notifications are disabled by user
                return Lunalify::Errors::Code::WINRT_DISBLED_BY_USER;

            default:
                // Unknown HRESULT
                LNF_DEBUG("Core:Error", "Unhandled HRESULT: 0x{:08X}", (uint32_t)hr);
    ...
    Lunalify::Errors::Code UpdateToastData(const std::string& appId, const std::string& tag, const std::string& title, const std::string& value, const std::string& vString, const std::string& status) {
        try {
            ...
            LNF_DEBUG("WinRT:Update", "Binding Keys: progressValue={}", value);
            ...
            LNF_ERROR("WinRT:Update", "Exception: {}", winrt::to_string(ex.message()));
            return Lunalify::Errors::Code::UNKNOWN;
        }
    }
    ...
    Lunalify::Errors::Code DispatchXmlToast(const std::string& appId, const std::wstring& xmlContent, const std::string& uniqueId, const std::string& initialTitle, const std::string& initialVal, const std::string& initialDisplay, const std::string& initialStatus) {
            ...
            try {
                ...
                LNF_ERROR("App:Toast:DispatchXmlToast", "XML Load failed for Id: {}", uniqueId);
                return Lunalify::Errors::Code::WINRT_INVALID_XML;
            }

            ToastNotification toast(doc);

            // Defines a tag for the notification
            ...
            toast.Activated([uniqueId](auto const& /*sender*/, winrt::Windows::Foundation::IInspectable const& args) {
                // Try to convert IInspectable to ToastActivatedEventArgs
                ...
                if (activatedArgs) {
                    ...
                    for (auto const& pair : userInputs) {
                        std::string key = winrt::to_string(pair.Key());
                        std::string val = winrt::to_string(winrt::unbox_value<winrt::hstring>(pair.Value()));
                        actionArgs += "|" + key + "=" + val;
                    }
                    LNF_INFO("App:Toast", "Activated Args with Inputs: {}", actionArgs);
                } else {
                    LNF_WARN("App:Toast", "Activated but no args found.");
                }


                LNF_INFO("App:Toast:DispatchXmlToast", "Interaction detected Id:{} Args: {}", uniqueId, actionArgs);
                ToastEventManager::PushEvent(uniqueId, "activated", actionArgs);
            });

        toast.Dismissed([uniqueId](auto const& /*sender*/, ToastDismissedEventArgs const& args) {
            ...
            LNF_INFO("App:Toast:DispatchXmlToast", "Toast dismissed, reason: {}", reason);
            ToastEventManager::PushEvent(uniqueId, "dismissed", reason);
        });
        ...
        switch (notifier.Setting()) {
            case NotificationSetting::Enabled:
                LNF_DEBUG("App:Toast:DispatchXmlToast", "Showing toast for uniqueId: {}", uniqueId);
                notifier.Show(toast);
                return Lunalify::Errors::Code::SUCCESS;

            // case NotificationSetting::QuietHours:
            //     LNF_WARN("App:Toast:DispatchXmlToast", "Active Quiet Hours, toast blocked Id: {}", uniqueId);
            //     return Lunalify::Errors::Code::WINRT_QUIET_HOURS;

            case NotificationSetting::DisabledForUser:
                LNF_WARN("App:Toast:DispatchXmlToast", "Disabled for user, toast blocked Id: {}", uniqueId);
                errorCode = Lunalify::Errors::Code::WINRT_DISBLED_BY_USER;
                break;

            case NotificationSetting::DisabledByGroupPolicy:
                LNF_WARN("App:Toast:DispatchXmlToast", "Disabled by group policy, toast blocked Id: {}", uniqueId);
                errorCode = Lunalify::Errors::Code::WINRT_DISABLED_BY_POLICY;
                break;

            case NotificationSetting::DisabledForApplication:
                LNF_WARN("App:Toast:DispatchXmlToast", "Disabled for application, toast blocked Id: {}", uniqueId);
                errorCode = Lunalify::Errors::Code::WINRT_REGISTRATION_MISSING;
                break;

            default:
                // Not certain what went wrong, return generic error
                LNF_ERROR("App:Toast:DispatchXmlToast", "Unknown Error, toast blocked Id: {}", uniqueId);
                errorCode = Lunalify::Errors::Code::UNKNOWN;
                break;
        }
        LNF_WARN("App:Toast", "Toast failed for {}. Error Code: {}", uniqueId, (int)errorCode);
        ToastEventManager::PushEvent(uniqueId, "failed", std::to_string((int)errorCode));
        return Lunalify::Errors::Code::UNKNOWN;
    }
}
