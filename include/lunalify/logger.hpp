...
public:
    ...
    static inline LogLevel ToLogLevel(const std::string& str) {
        if (str == "debug") return LogLevel::LEVEL_DEBUG;
        if (str == "info")  return LogLevel::LEVEL_INFO;
        if (str == "warn")  return LogLevel::LEVEL_WARN;
        if (str == "error") return LogLevel::LEVEL_ERROR;
        return LogLevel::LEVEL_INFO;
    }
    static inline LogLevel ToLogLevel(int level) {
        if (level == 1) return LogLevel::LEVEL_DEBUG;
        if (level == 2) return LogLevel::LEVEL_INFO;
        if (level == 3) return LogLevel::LEVEL_WARN;
        if (level == 4) return LogLevel::LEVEL_ERROR;
        return LogLevel::LEVEL_INFO;
    }
